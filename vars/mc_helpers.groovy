...
// checkMounted -- is the driveMounted -- expecting 4 inputs
def checkMounted(gInstance, gZone, gServiceAcct, gProject) {
    def chkMount = sh returnStdout: true, script: """
        gcloud compute ssh --project gInstance --zone gZone gServiceAcct@gProject \
        --command='cat /proc/mounts | grep minecraft | wc -l' """
    echo "Minecraft is mounted this many times: ${chkMount}"
    return chkMount
}